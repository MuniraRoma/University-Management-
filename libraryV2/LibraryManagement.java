...
public class LibraryManagement {
    ...
    // SINGLETON PATTERN: Using getInstance() instead of "new FileBookRepository()"
    // This ensures only one shared instance of the repository exists in the program.
    private static final BookRepository repository = FileBookRepository.getInstance();
    ...
    private static void borrowBook(String regNumber) {
        ...
        displayAvailableBooks(available);
        ...
        while (!valid) {
            try {
                ...
                if (numBooks > 0 && numBooks <= available.size()) {
                    List<Book> borrowed = selectBooksFromList(available, numBooks);
